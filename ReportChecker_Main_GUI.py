...
class mainGUI:
    def __init__(self):
        ...
        self.MmyIntf_Frame .grid(padx=10,pady=20,row=1,column=1)

        Label(self.MmyIntf_Frame, text="Memory:",font=("Arial",15)).grid(row=0,column=0)
        # Create a list of tuples that contains the text and values of the check buttons
        self.mainGUI_check_options = [("DDR_14L", "DDR_14L"),
                                      ("DDR_16L", "DDR_16L"), 
                                      ("DDR_18L", "DDR_18L"),  
                                    ("PCIe", "PCie"),
                                    ("XGMI", "XGMI"),
        ...
        for self.option in self.mainGUI_check_options:
            self.var = StringVar()
            self.mainGUI_MemIntf_Chkbox_vars.append(self.var)

        # Use a for loop to create the check buttons
        for i, (text, value) in enumerate(self.mainGUI_check_options):
            self.mainGUI_check_button = Checkbutton(self.MmyIntf_Frame, text=text, variable=self.mainGUI_MemIntf_Chkbox_vars[i], onvalue=value, offvalue=0, font=("Arial",15))
            if i < 3:
                self.mainGUI_check_button.grid(row=i+1,column=0,sticky="w")
                self.mainGUI_check_button.deselect()
                self.mainGUI_MemIntf_Chkbox_Btn.append(self.mainGUI_check_button)
            if 3<=i<=5:
        ...
        # 2)
        self.Main_BO_Chnl_width_DQ_entry_list[0].bind("<KeyRelease>", self.sync_main_entry)


        # Bind sync_background_entry to the KeyRelease event of entry
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[0].bind("<KeyRelease>", self.sync_background_entry)
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[1].bind("<KeyRelease>", self.sync_background_entry)
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[2].bind("<KeyRelease>", self.sync_background_entry)
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[3].bind("<KeyRelease>", self.sync_background_entry)

        RefDesignator_Gui_obj.RefDsn_BO_Chnl_width_DQ_entry_list[0].bind("<KeyRelease>", self.sync_background_entry)

    def update_checkbttn(self):
        self.mainGUI_MemIntf_Chkbox_Btn[0].select()
        RefDesignator_Gui_obj.SrcFile_MemIntf_Chkbox_Btn[0].select()
    ...
    def sync_main_entry(self, event):
        ...
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[0].delete(0, END)
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[0].insert(0, self.CPU0_input_entry.get())

        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[1].delete(0, END)
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[1].insert(0, self.CPU1_input_entry.get())

        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[2].delete(0, END)
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[2].insert(0, self.UsbHub_input_entry.get())

        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[3].delete(0, END)
        RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[3].insert(0, self.ClkBuff_input_entry.get())

        RefDesignator_Gui_obj.RefDsn_BO_Chnl_width_DQ_entry_list[0].delete(0, END)
        RefDesignator_Gui_obj.RefDsn_BO_Chnl_width_DQ_entry_list[0].insert(0, self.Main_BO_Chnl_width_DQ_entry_list[0].get())

    def sync_background_entry(self, event):
        # Update the value of sync_main_entry
        self.CPU0_input_entry.delete(0, END)
        self.CPU0_input_entry.insert(0, RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[0].get())

        self.CPU1_input_entry.delete(0, END)
        self.CPU1_input_entry.insert(0, RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[1].get())

        self.UsbHub_input_entry.delete(0, END)
        self.UsbHub_input_entry.insert(0, RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[2].get())

        self.ClkBuff_input_entry.delete(0, END)
        self.ClkBuff_input_entry.insert(0, RefDesignator_Gui_obj.CPU_Ref_input_entry_widgets[3].get())

        self.Main_BO_Chnl_width_DQ_entry_list[0].delete(0, END)
        self.Main_BO_Chnl_width_DQ_entry_list[0].insert(0,RefDesignator_Gui_obj.RefDsn_BO_Chnl_width_DQ_entry_list[0].get())
